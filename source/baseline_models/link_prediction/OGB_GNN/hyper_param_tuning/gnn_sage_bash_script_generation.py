...
available_gpus = ['6,1', '6,1']

# args dict
args_dict = {'device': 0, # gpu
           'log_steps': 1,
           'num_layers': 3,
           'hidden_channels': 256,
           'dropout': 0.0,
...
# define hyperparameter for grid search
hparam_dict = {
    'lr': [1.e-04, 1.e-03,1.e-02],  # np.logspace(-6, -3, 10),
    'num_layers': [2, 3, 4],
...
scripts_started = 0
with open('final_gnn_sage_parallelize.sh','w+') as f:
    ...
    for i in range(comb):
        ...
        f.write(f'CUDA_VISIBLE_DEVICES={available_gpus[i%len(available_gpus)]} python gnn.py --use_sage {param_str} &\n')
