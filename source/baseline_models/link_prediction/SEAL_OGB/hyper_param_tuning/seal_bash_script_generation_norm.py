...
available_gpus = ['4,7', '7,4']

# args dict
args_dict = {'log_steps': 1,
           'num_layers': 3,
           'hidden_channels': 32,
           'batch_size': 32,
           'lr': 1.e-03,
...
hparam_dict = {
    'lr': [1.e-04,1.e-05],  # np.logspace(-6, -3, 10),
    'num_layers': [2,3,4],
...
log_root = 'final_log_seal_norm_neurips'
...
scripts_started = 0
with open('final_seal_norm_parallelize.sh','w+') as f:
    ...
    for i in range(comb):
        ...
        f.write(f'CUDA_VISIBLE_DEVICES={available_gpus[i%len(available_gpus)]} python seal_link_pred.py --splitting_strategy spatial --use_feature {param_str} &\n')
