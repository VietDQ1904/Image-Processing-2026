...
from torch_geometric.nn import (GCNConv, SAGEConv, GINConv, CGConv,GENConv, GMMConv,SplineConv,NNConv,
...
class NN(torch.nn.Module):
    def __init__(self, hidden_channels, num_layers, max_z, train_dataset,
        ...
        if self.node_embedding is not None:
            initial_channels += node_embedding.embedding_dim
        self.conv1 = NNConv(initial_channels,hidden_channels,
            Sequential(
                Linear(self.num_edge_features, hidden_channels),
                ReLU(),
                Linear(hidden_channels, initial_channels*hidden_channels),
                ReLU(),
                BN(initial_channels*hidden_channels),
            ))
        self.convs = torch.nn.ModuleList()
        for i in range(num_layers - 1):
            self.convs.append(
                NNConv(hidden_channels,hidden_channels,
                    Sequential(
                        ...
                        ReLU(),
                        BN(hidden_channels*hidden_channels),
                    )))
...
class GMM(torch.nn.Module):
    def __init__(self, hidden_channels, num_layers, max_z, train_dataset,
                 use_feature=False, node_embedding=None, dropout=0.5,
                 jk=True, train_eps=False, kernel_size=25):
        ...
        if self.node_embedding is not None:
            initial_channels += node_embedding.embedding_dim
        self.conv1 = GMMConv(initial_channels,hidden_channels,self.num_edge_features,self.kernel_size)
        self.convs = torch.nn.ModuleList()
        for i in range(num_layers - 1):
            self.convs.append(
                GMMConv(hidden_channels,hidden_channels,self.num_edge_features,self.kernel_size)
                   )
