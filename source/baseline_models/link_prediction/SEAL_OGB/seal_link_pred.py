...
class SEALDataset(InMemoryDataset):
    def __init__(self, root, data, split_edge, num_hops, percent=100, split='train', 
                 ...
                 max_nodes_per_hop=None, directed=False, splitting_strategy='spatial', use_edge_feature=False):
        self.data = data
    ...
    def process(self):
        pos_edge, neg_edge = get_pos_neg_edges(self.split, self.split_edge, 
                                               ...
                                               self.data.num_nodes, 
                                               self.splitting_strategy, self.percent)
        ...
        if args.use_edge_attr:
            ...
            self.edge_attr = torch.tensor(self.data.edge_attr)
            self.edge = torch.tensor(self.data.edge_index)

            keys = [tuple([int(self.edge[0][i]), int(self.edge[1][i])]) for i in range(len(self.edge_attr))]
            values = [self.edge_attr[i, :] for i in range(len(self.edge_attr))]
        ...
        pos_list = extract_enclosing_subgraphs(
            pos_edge, A, self.data.x, self.edge_attr,1, self.num_hops, self.node_label,
            self.ratio_per_hop, self.max_nodes_per_hop, self.directed, A_csc,map,self.use_edge_feature)
        neg_list = extract_enclosing_subgraphs(
            neg_edge, A, self.data.x, self.edge_attr,0, self.num_hops, self.node_label,
            self.ratio_per_hop, self.max_nodes_per_hop, self.directed, A_csc,map,self.use_edge_feature)
...
class SEALDynamicDataset(Dataset):
    def __init__(self, root, data, split_edge, num_hops, percent=100, split='train', 
                 use_coalesce=False, node_label='drnl', ratio_per_hop=1.0, 
                 max_nodes_per_hop=None, directed=False, splitting_strategy='spatial', **kwargs):
        self.data = data
        self.split_edge = split_edge
        self.num_hops = num_hops
...
def test():
    ...
    pos_train_pred = train_pred[train_true==1]
    neg_train_pred = train_pred[train_true==0]
    ...
    if args.eval_metric == 'hits':
        ...
        results = evaluate_mrr(pos_train_pred,neg_train_pred,pos_val_pred, neg_val_pred, pos_test_pred, neg_test_pred)
...
def test_multiple_models(models):
    ...
    for data in tqdm(val_loader, ncols=70):
        ...
        for i, m in enumerate(models):
            logits = m(data.z, data.edge_index, data.batch, x, edge_weight, node_id, edge_attr)
...
def evaluate_mrr(pos_train_pred,neg_train_pred,pos_val_pred, neg_val_pred, pos_test_pred, neg_test_pred):
...
parser.add_argument('--use_edge_attr', action='store_true',
                    help="whether to consider ALL edge features in GNN")
# Training settings
...
if args.dataset.startswith('ogbl'):
    ...
    if args.use_edge_attr:
        data.edge_weight = torch.absolute(data.edge_attr[:, 2])

        # normalize edge features
        data.edge_attr[0,:] = torch.nn.functional.normalize(data.edge_attr[0,:], dim=0)
        data.edge_attr[1,:] = torch.nn.functional.normalize(data.edge_attr[1,:], dim=0)
        data.edge_attr[2,:] = torch.nn.functional.normalize(data.edge_attr[2,:], dim=0)
        data.edge_attr[3, :] = torch.nn.functional.normalize(data.edge_attr[3, :], dim=0)
        data.edge_attr[4, :] = torch.nn.functional.normalize(data.edge_attr[4, :], dim=0)
    ...
    raise ValueError('Please use the ogbl-datasets!')
...
if args.use_heuristic:
    ...
    if args.eval_metric == 'hits':
        results = evaluate_hits(pos_train_pred,neg_train_pred,pos_val_pred, neg_val_pred, pos_test_pred, neg_test_pred)
    elif args.eval_metric == 'mrr':
...
use_coalesce = True if args.use_edge_attr else False
...
for run in range(args.runs):
    ...
    if args.model == 'DGCNN':
        ...
        model = CG(args.hidden_channels, args.num_layers, max_z, train_dataset,
                   args.use_feature, node_embedding=emb).to(device)
    elif args.model == 'NN':
        model = NN(args.hidden_channels, args.num_layers, max_z, train_dataset,
                   args.use_feature, node_embedding=emb).to(device)
    elif args.model == 'GMM':
        model = GMM(args.hidden_channels, args.num_layers, max_z, train_dataset,
                    args.use_feature, node_embedding=emb, kernel_size=25).to(device)
