...
def k_hop_subgraph(src, dst, num_hops, A, sample_ratio=1.0, 
                   ...
                   y=1, directed=False, A_csc=None,edge_features=None,map=None):
    ...
    if edge_features is not None:
        # assemble (node1id,node2id) similar scipy.sparse.csr_matrix notation
    ...
    return nodes, subgraph, dists, node_features, y, edge_features
...
def construct_pyg_graph(node_ids, adj, dists, node_features, y, edge_features,node_label='drnl',use_edge_feature=False):
    ...
    if use_edge_feature:
        edge_attr = edge_features.t() # necessary due to collate
        data = Data(node_features, edge_index, edge_attr,edge_weight=edge_weight, y=y, z=z,
...
def extract_enclosing_subgraphs(link_index, A, x, edge_attr,y, num_hops, node_label='drnl',
...
def get_pos_neg_edges(split, split_edge, edge_index, num_nodes, splitting_strategy,percent=100):
    if 'edge' in split_edge['train']:
        pos_edge = split_edge[split]['edge'].t()
        if split == 'train':
            if splitting_strategy == 'seal_random':
...
# cf. https://neo4j.com/docs/graph-algorithms/current/labs-algorithms/linkprediction/



## from Julian 
def TN(A, edge_index, batch_size=100000):
    # The Common Neighbor heuristic score.
    link_loader = DataLoader(range(edge_index.size(1)), batch_size)
    scores = []
    for ind in tqdm(link_loader):
