...
def train(model, loader, optimizer, device, weights):
    ...
    for data in loader:
        ...
        y = data.y.squeeze(1)
        loss = F.nll_loss(out[data.train_mask], y[data.train_mask], weight=weights)
        loss.backward()
        optimizer.step()
...
def test(model, data, evaluator, subgraph_loader, device):
    ...
    return train_acc, valid_acc, test_acc, y_pred[data.test_mask], y_true[data.test_mask]
...
def main():
    ...
    parser = argparse.ArgumentParser(description='OGBN GraphSAINT')
    ...
    parser.add_argument('--lr', type=float, default=1.e-03)
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            loss = train(model, loader, optimizer, device, weights)
