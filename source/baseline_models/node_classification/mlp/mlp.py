...
def train(model, x, y_true, train_idx, optimizer, weights):
    model.train()

    optimizer.zero_grad()
    out = model(x[train_idx])
    loss = F.nll_loss(out, y_true.squeeze(1)[train_idx], weight=weights)
    loss.backward()
    optimizer.step()
...
@torch.no_grad()
def extended_test(model, x, y_true, split_idx, metric_dict, unique_labels):
    model.eval()
    out = model(x)
    y_pred = out.argmax(dim=-1, keepdim=True)
...
def main():
    ...
    x = data.x
    if args.use_node_embedding:
        file_name = f'{args.dataset}_embedding.pt'
        embedding = torch.load(file_name, map_location='cpu')
    ...
    for run in range(args.runs):
        ...
        if args.test_only:
            metric_res_dict = extended_test(model, x, y_true, split_idx, metric_dict, unique_labels)
        ...
        for epoch in range(1, 1 + args.epochs):
            loss = train(model, x, y_true, train_idx, optimizer, weights)
